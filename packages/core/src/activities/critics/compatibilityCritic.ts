...
export async function compatibilityCritic(state: WorkflowState, deps: ActivityDeps): Promise<CriticFinding[]> {
  ...
  for (const diff of state.diffProposal.diffs) {
    ...
    if (isEntityFile(diff.filePath) && containsFieldRename(diff.diffContent)) {
      findings.push({ critic: 'compatibility', severity: 'critical', file: diff.filePath, line: null, ruleId: 'SCHEMA_MIGRATION_REQUIRED', message: 'Field renamed in entity — database migration required', resolutionHint: 'Add a Flyway/Liquibase migration script to rename the column.' });
    }
  }
  return findings;
}
