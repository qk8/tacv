  ...
  for (const dep of added) {
    try {
      const res = await fetch('https://api.osv.dev/v1/query', {
        ...
        body: JSON.stringify({
          package: { name: dep.name, ecosystem: dep.ecosystem === 'npm' ? 'npm' : 'Maven' },
          version: dep.version,
        }),
      ...
      if (data.vulns && data.vulns.length > 0) {
        for (const vuln of data.vulns.slice(0, 3)) {
          ...
          // Use plugin-delegated manifest file name (no more if(langId === 'java'))
          findings.push({
            critic: 'dependency_vuln',
            severity: severity === 'CRITICAL' ? 'critical' : 'warning',
            file: getDependencyFile(plugin), line: null,
            ruleId: `OSV-${vuln.id}`,
            message: `${dep.name}@${dep.version}: ${vuln.summary}`,
            resolutionHint: fixed ? `Upgrade to ${fixed}` : 'No fixed version available — consider an alternative.',
          });
        }
      }
